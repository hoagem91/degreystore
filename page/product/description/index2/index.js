...
addToCartButtons.forEach((button) => {
  button.addEventListener("click", (e) => {
    e.preventDefault();
    const product = button.closest(".description-product");
    const productInfo = {
      id: Date.now(),
      name: product.querySelector("h1").textContent,
      price: product.querySelector(".price-cost").textContent,
      image: product.querySelector(".mySlides").src,
      number: product.querySelector(".quantity-input").value,
    };
...
function updateCartUI() {
  ...
  // Tính tổng tiền - đã cập nhật để nhân với số lượng
  const total = cart.reduce((sum, item) => {
    const price = parseInt(item.price.replace(/[^\d]/g, ""));
    const quantity = parseInt(item.number) || 1; // Lấy số lượng, mặc định là 1 nếu không có
